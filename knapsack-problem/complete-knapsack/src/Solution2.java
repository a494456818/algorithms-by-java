...
public class Solution2 {
    ...
    public int complteKnapsack(int[] w, int[] v, int C) {
        ...
        for (int i = 1; i < n; i++)
            for (int j = 0; j < C + 1; j++) {
                memo[i % 2][j] = memo[(i - 1) % 2][j];// 不选择物品i
                if (j - w[i] >= 0) {
                    int temp1 = v[i] + memo[(i - 1) % 2][j - w[i]]; //选择物品i并选择物品i-1
                    int temp2 = v[i] + memo[i % 2][j - w[i]]; //选择物品i并选择物品i
                    memo[i % 2][j] = Math.max(memo[i % 2][j], Math.max(temp1, temp2));
                }
            }
